...
export function createTicket({ title, description, category, priority, requester, aiCategorized }) {
  ...
  // Alert agents — for urgent tickets only.
  if (priority === 'urgent') {
    notifyAgents(ticket.id, `Urgent: ${ticket.id} raised in ${category} — "${ticket.title}".`);
  }
  persist();
  return ticket;
}
...
// Apply a status (and optional resolution) to many tickets at once — used to
// resolve a whole duplicate group in one action.
export function bulkUpdate(ids, status, resolution, agent) {
  const updated = [];
  for (const id of ids || []) {
    const t = getTicket(id);
    if (!t) continue;
    if (resolution) t.resolution = resolution;
    if (STATUSES.includes(status)) updateStatus(id, status, 'Resolved as part of a duplicate group', agent);
    updated.push(t.id);
  }
  return updated;
}
...
// Team-wide notification for agents (used for urgent tickets only).
function notifyAgents(ticketId, message) {
  db.notifCounter += 1;
  db.notifications.push({
    id: db.notifCounter,
    audience: 'agent',
...
export function listNotifications({ requester, audience } = {}) {
  return db.notifications
    .filter((n) => audience
      ? (n.audience || 'employee') === audience
      : ((n.audience || 'employee') === 'employee' && (!requester || n.requester === requester)))
    .sort((a, b) => new Date(b.at) - new Date(a.at));
}

export function markNotificationsRead({ requester, audience } = {}) {
