...
// ---------------------------------------------------------------------------
// groupDuplicates() — cluster open tickets that are the SAME complaint AND the
// SAME urgency, so an agent can resolve a whole group in one go.
// ---------------------------------------------------------------------------
export function groupDuplicates(tickets, threshold = 0.45) {
  const open = tickets.filter((t) => t.status === 'Open' || t.status === 'In Progress');
  const clusters = [];
  for (const t of open) {
    const vec = termFreq(tokenize(`${t.title} ${t.description}`));
    let placed = false;
    for (const c of clusters) {
      if (c.category === t.category && c.priority === t.priority && cosine(vec, c.vec) >= threshold) {
        c.tickets.push(t); placed = true; break;
      }
    }
    if (!placed) clusters.push({ category: t.category, priority: t.priority, title: t.title, vec, tickets: [t] });
  }
  return clusters
    .map((c) => ({
      category: c.category, priority: c.priority, title: c.title, size: c.tickets.length,
      tickets: c.tickets.map((t) => ({ id: t.id, title: t.title, requester: t.requester, status: t.status })),
    }))
    .sort((a, b) => b.size - a.size || a.category.localeCompare(b.category));
}
// knowledge base each call (so it "keeps learning" as tickets arrive) and
// answers in plain, layman-friendly language strictly within what it found.
// ---------------------------------------------------------------------------
const QWORDS = new Set(['how', 'who', 'why', 'where', 'when', 'does', 'did', 'can', 'should', 'would',
  'are', 'was', 'were', 'has', 'had', 'get', 'got', 'usually', 'people', 'someone', 'anyone']);

export async function ragAnswer({ question = '' }, tickets = []) {
  const q = String(question || '').trim();
  if (!q || isGibberish(q)) {
    return { reply: "That doesn't look like a question I can help with — try asking in a few plain words, like “how do we usually fix VPN drops?”.", sources: [] };
  }
  const docs = [
    ...tickets.map((t) => ({
      type: 'ticket', id: t.id, title: t.title,
      text: `${t.title}. ${t.description} ${t.resolution || ''}`,
      meta: `${t.category} · ${t.status}`, resolution: t.resolution || '',
    })),
    ...ARTICLES.map((a) => ({
      type: 'doc', id: a.id, title: a.title,
      text: `${a.title} ${a.keywords.join(' ')} ${a.steps.join(' ')}`,
      meta: a.dept, steps: a.steps,
    })),
  ];
  // Drop stop/question words so common words don't create spurious matches.
  const qVec = termFreq(tokenize(q).filter((w) => !STOP.has(w) && !QWORDS.has(w)));
  const ranked = docs
    .map((d) => ({ d, score: cosine(qVec, termFreq(tokenize(d.text))) }))
    .filter((r) => r.score > 0.07)
    .sort((a, b) => b.score - a.score)
    .slice(0, 5);

  // Out of knowledge range — don't guess on a single weak token overlap.
  if (!ranked.length || ranked[0].score < 0.13) {
    return { reply: "I don't have anything on that in the current tickets or help docs yet. Try rephrasing, or ask about something an employee might raise a ticket for.", sources: [] };
  }
  const sources = ranked.map((r) => ({ id: r.d.id, title: r.d.title, type: r.d.type, match: Math.round(r.score * 100) }));

  if (!client) {
    const top = ranked[0].d;
    const reply = top.type === 'doc'
      ? `${top.title}: ${top.steps.slice(0, 3).join(' ')}`
      : top.resolution
        ? `Closest match is ${top.id} — “${top.title}”. It was resolved by: ${top.resolution}`
        : `The closest item I found is ${top.id} — “${top.title}” (${top.meta}). There's no recorded resolution yet.`;
    return { reply, sources };
  }

  try {
    const context = ranked.map((r) => `[${r.d.type}:${r.d.id}] ${r.d.title} — ${r.d.text.slice(0, 320)}`).join('\n');
    const res = await client.messages.create({
      model: MODEL,
      max_tokens: 500,
      system:
        'You are a helpful internal support knowledge assistant. Answer the question in clear, simple, ' +
        'layman-friendly language using ONLY the provided context from support tickets and help articles. ' +
        'If the context does not contain the answer, say you don\'t have that information yet — do not guess. ' +
        'Reference ticket or article ids in brackets when useful. Keep it short and practical.',
      messages: [{ role: 'user', content: `Question: ${q}\n\nContext:\n${context}` }],
    });
    const block = res.content.find((b) => b.type === 'text');
    return { reply: block ? block.text.trim() : ranked[0].d.title, sources };
  } catch (err) {
    console.warn('RAG failed, using heuristic:', err.message);
    const top = ranked[0].d;
    return { reply: `Closest match: ${top.id} — “${top.title}” (${top.meta}).`, sources };
  }
}
